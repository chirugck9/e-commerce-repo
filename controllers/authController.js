...
exports.signup = async ( req, res ) => {
    try {
        ...
        return res.status(201).json({ message: "User created successfully", user: { uid: firebaseUser.uid, email, role }});
    } catch (error) {
        console.error(error);
        return res.status(400).json({ error: "Error creating user", message: error.message });
    }
}
...
exports.login = async ( req,res ) => {
        ...
        try {
            ...
            return res.status(200).json({ message: "Logged in successfully.", idToken, role });

        } catch (error) {
            console.error("Error:", error);
            return res.status(400).json({ error: "Authentication failed", message: error.message });
        }
}